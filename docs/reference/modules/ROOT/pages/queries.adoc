...
`(sum ?lvar)` accumulates as single value via the Clojure https://clojuredocs.org/clojure.core/+[`&#43;`] function

`(min ?lvar)` and `(max ?lvar)` return a single value via the Clojure https://clojuredocs.org/clojure.core/compare[`compare`] function which may operates on many types (integers, strings, collections etc.)
